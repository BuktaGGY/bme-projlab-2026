...
public class Prototipus {
    ...
    try {
        switch (command) {
            ...
            case "stat":
                ...
                if (statId.equals("kassza")) {
                    ...
                    if(jk.getForgalomIranyito().ContainsJarmuId(statId)) {
                        jk.getForgalomIranyito().getJarmu(statId).statKiir();
                    }
                    if(poik.containsKey(statId)) {
                        poik.get(statId).StatKiir();
                    }
                }
                break;
            ...
            case "stat_minden":
                ...
                for (Auto stat_auto : autok.values()) stat_auto.statKiir();
                for (Busz stat_buszok : buszok.values()) stat_buszok.statKiir();
                for (Hokotro stat_hokotrok : hokotrok.values()) stat_hokotrok.statKiir();
                break;
    ...
    public void run() {
        ...
        while (scanner.hasNextLine()) {
            ...
            if (!egyParancsFeldolgozasa(line, scanner)) {
                break; 
            }
        }
